...
//Если парсер не смог открыть нашу ссылку (иногда он это делает со второй попытки), то отправляем ещё раз
        if (isset($result['error_code'])) {
            ...
            if (isset($result['error_code'])){
                ...
                TelegrammLoging::send('АO P Ошибка открытия ссылки парсером со второй попытки, задача '.$this->task->id. ' обьявление: '.$this->task->ad->id." group:".$this->group->id." stacktrace: ".json_encode($result). "params :".json_encode($params));
            }else{
                ...
                TelegrammLoging::send('АO P Открытие ссылки парсером со второй попытки, задача '.$this->task->id. ' обьявление: '.$this->task->ad->id." group:".$this->group->id." stacktrace: ".json_encode($result). "params :".json_encode($params));
            }
        }else{
            $this->task->status = AutopostingTasks::STATUS_POSTED;
            $this->task->save();
            \Yii::warning('АO P Открытие ссылки парсером со первой попытки, задача '.$this->task->id, "DEBUG");
            TelegrammLoging::send('АO P Открытие ссылки парсером со первой попытки, задача '.$this->task->id. ' обьявление: '.$this->task->ad->id." group:".$this->group->id);
        }
    }
