          ...
          <tbody>
            <?php foreach ($countries as $country) : ?>
              <tr>
                ...
                <td><a href="<?php echo Url::toRoute(['regions/','country_id' => $country->id])?>">
                        <?php echo $country->countryText->name?>
                    </a>
                    <?= backend\helpers\ActiveLabel::status($country->active, [
                        'active' => 'активно',
                        'inactive' => 'не активно'
                    ])?>
                </td>
                <td><?php echo $country->domain?>
                <td>
                    <span data-placement="top" data-toggle="tooltip" title="Редактировать">
                        <button class="btn btn-primary btn-xs loadcontent"
                                data-link="<?= Url::toRoute(['edit','id' => $country->id])?>">
                            <span class="glyphicon glyphicon-pencil"></span>
                        </button>
                    </span>
                    <span data-placement="top" data-toggle="tooltip" title="Удалить">
                        <button class="btn btn-danger btn-xs senddata"
                                data-link="<?= Url::toRoute(['delete','id' => $country->id])?>">
