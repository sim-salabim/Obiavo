    ...
    'rows' => [
        [
            ...
            'attributes' => [
                ...
                ['name' => 'group_id','type' => Form::INPUT_TEXT,'label' => 'ID соцгруппы','model'=>$sn_group, 'label_desc' => 'Access Token для Twitter или Instagram username' ],
                ['name' => 'token','type' => Form::INPUT_TEXT,'label' => 'Токен группы','model'=>$sn_group,'label_desc' => 'Access Token Secret для Twitter или пароль instagramm. Access token для ok.ru ' ],
                ['name' => 'consumer_key','type' => Form::INPUT_TEXT,'label' => 'Consumer key','model'=>$sn_group,'label_desc' => 'Только для Twitter. Публичный ключ приложения для ok.ru. App ID для Facebook' ],
                ['name' => 'consumer_secret','type' => Form::INPUT_TEXT,'label' => 'Consumer secret','model'=>$sn_group,'label_desc' => '"Consumer secret" для Twitter. Секретный ключ приложения для ok.ru. App Secret для Facebook' ],
                ...
                ['name' => 'regions_id','type' => Form::SEARCH_AUTOCOMPLETE,'label' => 'Регион', 'model_name' => 'SocialNetworksGroups', 'model'=>$sn_group, 'url' => 'regions/search ', 'placeholder' => 'Начните печатать название региона...', 'current_value' => $current_region_value, 'input_id' => 'socialnetworksgroups-regions_id', 'previous_input' =>
                    ['id'=>'hidden-socialnetworksgroups-countries_id', 'query_name'=>'country_id']],
                ['name' => 'cities_id','type' => Form::SEARCH_AUTOCOMPLETE,'label' => 'Город', 'model_name' => 'SocialNetworksGroups', 'model'=>$sn_group, 'url' => 'cities/search ', 'placeholder' => 'Начните печатать название города...', 'current_value' => $current_city_value, 'input_id' => 'socialnetworksgroups-cities_id', 'previous_input' =>
                    ['id'=>'hidden-socialnetworksgroups-regions_id', 'query_name' => 'region_id']
                ],
